...
In order to get in-depth analysis of California's varied climate regions, here we divide the state into 5 regional zones as Figure \ref{fig:Figure1} shows. Simulations and datasets are masked to get climate features in each region. First year of simulations was treated as model spin-up, thus, all the data analysis are based on the period from 1980 to 2005, i.e. 26 years.
...
%The underlying reasons behind those differences can be manifold including the models' inner mechanism, the forcing data and the scale effects. 
...
\textbf{varres-CESM 0.125d} & 0.651$\     $	2.857	& 0.233$\     $	1.687$\     $	0.607 &	0.824	$\     $ 2.752\\				%\textbf{Uniform CESM 0.25d} & -0.409 $\     $	1.402	& -0.672	$\     $ 0.312	$\     $ -0.907	& -0.361$\     $	1.272 \\
...
\end{center}
\caption{Domains of WRF simulations and five climate divisions in California. {\color{red} to be modified. }} \label{fig:Figure1}
\end{figure}
...
\end{center}
\caption{JJA average daily Tmax from models and reference datasets, and differences between them (unit: C).} \label{fig:Figure4}
\end{figure}
...
\end{center}
\caption{As Figure 4, but for summer Tmin.} \label{fig:Figure5}
\end{figure}
...
\end{center}
\caption{As Figure 4, but for summer Tavg.} \label{fig:Figure6}
\end{figure}
...
%add a table for four seasons (mean, variance)
...
\end{center}
\caption{Climatology simulation comparison between varres-CESM 0.25 deg and uniform CESM 0.25 deg} \label{fig:Figure16}
\end{figure}
