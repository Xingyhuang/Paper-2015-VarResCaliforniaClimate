...
The fully compressible non-hydrostatic WRF-ARW model in version 3.5.1 is used. ERA-Interim surface and pressure-level reanalysis was used to provide initial and lateral conditions for the domains. The lateral conditions and SSTs were updated every 6 hours. ERA-Interim reanalysis ($\sim$80 km) has been widely used and validated for its reliability as forcing data \citep{dee2011era}. Two simulations are conducted with horizontal resolution of 27km (WRF27) and 9km (WRF9) respectively, over the time period 1979-01-01 to 2005-12-31 (UTC). The year 1979 is used as a spin-up period and is discarded for purposes of analysis. The $\sim$10 km resolutions are actually finer than most previous studies for long-term climate.

The simulation domains of WRF9 are depicted in Figure \ref{fig:wrf_domains}. For the WRF27 simulation, one domain is used.  For the WRF9 simulation, two nested domains are used with the outer domain at 27km (same as the WRF27) and inner domain at 9km horizontal grid resolution. Two-way nesting is enabled.  These choices have been made to satisfy the natural WRF aspect ratio of 3:1.  Both grids are centered on CA and have respectively, 120$\times$110 and 151$\times$172 grid points. Around the boundaries, 10 grid points are used as lateral relaxation zones. In order to reduce the drift between forcing data and RCM, grid nudging \citep{stauffer1990use} was applied to the outer domain every 6 hours at all levels except the planetary boundary layer (PBL) as suggested by \cite{lo2008assessment}. This setup uses 41 vertical levels with model top pressure at 50 hPa.
...
For consistency, the reference datasets are averaged to the given model output resolution when differing and comparing statistical values such as root mean square error (RMSE), bias, and correlation. These datasets are remapped using a bilinear interpolation method, which has been tested to be satisfactory without notable difference from original datasets comparing with other interpolation methods like patch.
...
{\color{red}computational cost is masked here as Colin suggested above}
...
Based on 26 years of high-resolution historical climate simulation data over California, we analyzed both temperature and precipitation in California and across its climate divisions. We found that varres-CESM output provides some improvement over WRF for mean climatology, although the model still exhibited slight biases towards a warmer and moister climate, especially in the Central Valley. WRF exhibited a clear cold bias in summer Tmax over most regions except the Central Valley, but an essentially opposite bias in summer Tmin, especially for the finer resolution simulation.  However, looking at the magnitude of RMSE, WRF was better overall at modeling Tmin. WRF also presented smaller daily temperature variability than both varres-CESM and the observational datasets. Higher resolution (0.125$^\circ$) simulations with varres-CESM were quite similar to the coarser resolution runs, although there were some improvements in capturing summer maximum temperature and precipitation. However, WRF 27km showed much better performance than WRF 9km, especially in modeling Tmax and precipitation trends. WRF 27km showed a slightly dryer climate, but WRF 9km dramatically overestimated precipitation over the North coastal region and the mountain region. Previous studies have also found this phenomenon for fine-scale simulations using RCMs as aforementioned. Additionally, we found that almost all of the precipitation comes from resolved (large-scale) processes for all these models, though convective scheme can also affect the large-scale microphysics results.
...
Generally, when compared with observations, simulations capture regional climatology patterns. Spatial correlations are high between these simulations and observations. Errors are not indicative of deep underlying problems with the model formulation, but one should be careful in using these models for assessing future climate change. Uncertainty between observational datasets exists, but is much smaller than the modelsÕ biases. Compared with the uniform resolution CESM-FV simulation, varres-CESM performed similarly or even better in some cases. In summary, varres-CESM demonstrated comparable ability in studying high-resolution regional climatology in comparison to a regional climate model (WRF) and a uniform high-resolution GCM (CESM-FV). This will add the value to the use of variable-resolution GCMs in assessing climate change over the coming century.
...
\begin{table}
\caption{Reanalysis and statistically downscaled observational datasets} \label{tab:Datasets}
\begin{center}
\begin{tabular}{lcccc}
...
\end{tabular}
\begin{tabular}{p{6in}}
\textbf{Notes:}  SON (September-October-November), MAM (March-April-May); CV (central valley), Mt (Mountain), NC (North coast), SC (South coast), Dt (Desert).
\end{tabular}
\end{center}
\end{table}
...
\end{center}
\caption{Topography in meters (m) for (top left to bottom right) varres-CESM 0.25$^\circ$, varres-CESM 0.125$^\circ$, uniform CESM-FV 0.25$^\circ$, WRF 27km, WRF 9km and ERA-Interim ($\sim$80 km).} \label{fig:topo} {\color{red} check ERA-Interim}
\end{figure}
...
(Daymet is similar to PRISM, and isn't shown in the difference plot).} \label{fig:t2max_JJA}
\end{figure}
...
(Daymet is similar to UW, so not showed in difference plot)} \label{fig:t2min_JJA}
\end{figure}
...
\end{center}
\caption{As Figure 4, but for winter Tmin.} \label{fig:t2min_DJF}  {\color{red} mask out ocean}
\end{figure}
...
\end{center}
\caption{Annual DJF precipitation from models and reference datasets (mm/d).} \label{fig:pr_DJF} {\color{red} mask out ocean}
\end{figure}
...
\end{center}
\caption{Climatology simulation comparison between varres-CESM 0.25 degree and uniform CESM-FV 0.25 degree.} \label{fig:vr_uni_CESM}
\end{figure}
