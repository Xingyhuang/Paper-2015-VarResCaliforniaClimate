...
Variable-resolution topography files were produced by sampling the National Geophysical Data Center (NGDC) 2-min ($\sim$ 4 km) Gridded Global Relief Dataset (ETOPO2v2) topography dataset, followed by application of a differential smoothing technique as described in \cite{zarzycki2015effects}.  Using this technique, the $c$ parameter from Eq. (1) was adjusted to reduce noise in the vertical pressure velocity field. The grid-scale topography is depicted in Figure \ref{fig:topo}. The topography of uniform CESM at 1 degree is also included in the figure. As we can see, higher resolution clearly provides a much improved representation of regional topography, which is necessary for the correct treatment of fine-scale dynamic processes strongly influenced by complex terrain. And topography at very coarse degree ($\sim$1$^\circ$) cannot describe any local details like the shape of valley or the range of mountain, resulting the loss of the regionally climatological patterns.
...
Additionally, the following physics parameterizations were employed: WSM (WRF Single-Moment) 6-class graupel microphysics scheme \citep{hong2006wrf}, Kain-Fritsch cumulus scheme \citep{kain2004kain}, CAM shortwave and longwave radiation schemes \citep{collins2004description}.  These setting were chosen by assessing the results from several common parameterization combinations over a one-year simulation period, as compared with gridded observations. For the boundary layer, the Yonsei University scheme (YSU) \citep{hong2006new} and the Noah Land Surface Model \citep{chen2001coupling} were used. Both were chosen as they are common for climate applications that balance long-term reliability and computational cost.  Although many other options and combinations of parameterizations are available for configuring WRF (and others have tackled a complete assessment of these options for particular problems), this choice was made simply to represent a typical RCM configuration.
...
To assess differences in these data products, we have calculated the MSD values between the above reference datasets for seasonally averaged JJA Tmax, Tmin and DJF Pr over the five sub-zones and tabulated these results in Table \ref{tab:stat_Datasets}. Student's t-test was employed to determine if differences were statistically significant. For minimum and maximum temperature, gridded observational datasets were different from each other over some regions.  The most pronounced differences occurred in the NC region, with MSD values reaching up to $\sim$ 4$^\circ C$, although differences were also apparent for MR Tmin. Clearly UW and Daymet have a colder climatology than PRISM. NARR, as a reanalysis dataset, is different from the others over most regions, with overall larger Tmin and smaller Tmax. For precipitation, essentially no significant differences were present between datasets, especially among PRISM, UW and Daymet. NARR and CPC (not shown) seem to have slightly lower precipitation values than others.
...
The spatial standard deviation of JJA Tmax, Tmin and Tavg from models and PRISM is presented in Figure \ref{fig:t2_JJA_std}. It can be seen that the variability is largely consistent across different sub-zones, and the values are around 0.5 to 1.5 $^\circ C$ for all the datasets, except for the high Sierras in the WRF 9km simulation which show enhanced variability ($\sim$2$^\circ C$).

%From the RMSD values shown in Table \ref{tab:stat_std_JJA_t2}, varres-CESM and WRF 27km have smaller deviations ($\sim$0.1-0.2$^\circ C$) from observations comparing with WRF 9km ($\sim$0.2-0.3$^\circ C$).
...
Along the western edge of the Sierra Nevada and into the CV, varres-CESM overestimates total precipitation relative to PRISM (see MRD in table \ref{tab:stat_Pr}), especially the coarser resolution (28 km) simulation ($\sim$ 40$\%$-50$\%$).  This difference is statistically significant compared to PRISM at the 95\% level. Varres-CESM 0.125$^\circ$ performs better, and does not show a statistically significant deviation over the aforementioned region. On the other hand, precipitation is slightly underestimated relative to PRISM along the North Coast (a statistically significant difference), particularly near the Oregon border. There are also notable differences between WRF 27km and WRF 9km. WRF 27km underestimates precipitation along the North Coast (by about 30$\%$) but fairly accurately captures precipitation in the CV, whereas WRF 9km greatly overestimates precipitation (by about 60$\%$-80$\%$) along the North coast and the Sierra Nevada (see MRD in table \ref{tab:stat_Pr}). From Table \ref{tab:stat_Pr} as a guide, varres-CESM 0.125$^\circ$ performs slightly better than CESM 0.25$^\circ$ and WRF 27km with RMSD values around 1.2 mm$/$d over DJF. Varres-CESM, especially at higher resolution, has smaller MSD than WRF 27km. The MRD is about 0.25 to 0.35 for varres-CESM and WRF 27km, and is about 0.65 to 0.85 for WRF 9km in DJF. In fact, varres-CESM 0.125$^\circ$ is statistically equivalent to PRISM everywhere except over part of north coast, where uncertainty also tends to be largest. WRF 9km is significantly different from PRISM over the mountain region and part of north coast. 

%WRF 27km is significantly different from PRISM  over coastal region and eastern side of the mountain region. Varres-CESM 0.25d is significantly different from PRISM  over CV region and part of north coast region. {\color{red}Models at coarser resolution did not show organized patterns of similarity [What do you mean?]}. %In general, the relatively notable differences from Figure \ref{fig:pr_DJF_Anuual} also result statistically significant difference. 
...
Interannual variability of precipitation is calculated here as the standard deviation of annual and DJF precipitation and depicted in Figure \ref{fig:pr_DJF_Annual_std}.  In general, precipitation variability exhibits a similar pattern to the precipitation intensity. Models capture the variation of precipitation well and closely match PRISM, although varres-CESM 0.125deg and WRF 27 km provide the closest match.  Standard deviation is $\sim$50$\%$ higher for WRF 9km, consistent with overestimated precipitation intensity.  Varres-CESM 0.25$^\circ$ also tends to overestimate variability in the southern Sierra Nevada mountains, likely due to enhanced orographic uplift from the relatively coarse topography (relative to 0.125$^\circ$).

%(as proved by Table \ref{tab:stat_Pr_std})

The annual cycle of precipitation averaged over each sub-zone of the simulation period is presented in Figure \ref{fig:trd_pr_allzones}. These values are calculated by taking the average of mean monthly Pr over specific regional zone. The 95\% confidence intervals of UW and PRISM are also depicted indicating statistically significant differences when simulation results are out of the highlighted region. It can be seen that all simulations exhibit similar trends to the reference datasets, with highest precipitation occurring over winter and lowest values over summer. Nonetheless, the strongest deviations occur during the rainy seasons, especially in winter. WRF 27km is drier than PRISM and UW with relative differences ranging from $\sim$10$\%$-40$\%$, whereas WRF 9km is far wetter with relative differences reaching up to 40$\%$-80$\%$ over these five climate sub-zones. Varres-CESM tracks well with observed precipitation with $\sim$10$\%$-20$\%$ relative difference everywhere except in the CV, where precipitation is overestimated at rainy seasons with about 70$\%$-80$\%$. From the MWW test, varres-CESM and WRF 27km are not significantly different from reference datasets in most regions, except over the CV in late winter to spring (for varres-CESM 0.25$^\circ$), and the NC winter and spring, and DR's winter (for WRF 27km). WRF 9km is significantly different from the reference datasets over most regions, except DR and SC's winter and spring.
...
The monthly cycle sample standard deviation is depicted in Figure \ref{fig:trd_pr_allzones_std}. The variability in observations has a similar monthly trend compared to precipitation rate, with overall values from 0 to 4 mm$/$day.  Generally higher inter-annual variability occurs over locations of higher mean precipitation \ref{fig:trd_pr_allzones}. Compared with observations, varres-CESM exhibited basically no more than 1mm$/$day larger variability in the rainy season except over the CV. WRF 27km was also close to observations with generally no more than 1mm$/$day over all regions. WRF 9km again showed enhanced variability ($\sim$1.5 mm$/$day more) during the wintertime over most regions. Overestimated variability and precipitation intensity has also been found in previous studies; for example,\cite{duffy2006simulations} observe higher variability associated with higher spatial resolution in RCMs, attributed to a more accurate representation of topography. The main cause of the interannual variability of precipitation over CA is the El Ni\~{n}oÐSouthern Oscillation (ENSO), which varies the amount of moisture flux transported to this region. To assess deviations from observations, the Levene test was used in place of the F-test due to non-normality of precipitation. Under this test WRF 9km was significantly different from both reference datasets over summer and winter periods in MR, and over July to September in DR and SC.  Varres-CESM 0.25$^\circ$ was significantly different within the CV over the  winter season.
...
The overestimation of precipitation for WRF at high resolution has also been found in previous studies. Although not as pronounced, \cite{caldwell2009evaluation} demonstrated that WRF at 12km largely overestimated the precipitation over the California's mountainous regions (however, this paper did employ a different set of parameterizations).  \cite{caldwell2009evaluation} argue that the bias comes from a variety of sources, rather than simply the different choice of parameterizations. The exact cause of this overprediction has yet to be identified in the literature and a comprehensive analysis of the cause of these errors is beyond the scope of this paper. Further discussion can be found in former studies that employ different microphysics schemes (and so produce a wide range of precipitation magnitudes) including \citep{leung2003hydroclimate, jankov2005impact, gallus2006comparison, chin2010preliminary, caldwell2010california}.
...
Deviations from reference datasets do exist in these simulations, but they have different features. During the summer period, varres-CESM produced a modeled climate about 2 to 3$^\circ C$ warmer than observations, especially in the Central Valley. On the other hand, WRF exhibited a colder ($\sim$2$^\circ C$) Tmax over most regions except the Central Valley, but a little warmer in Tmin. Overall, varres-CESM was more accurate in reproducing mean climatology of Tmax, whereas WRF was better at modeling Tmin and Tavg. The interannual standard deviation of JJA mean temperature is largely within the range of 0.5 to 1.5$^\circ C$. WRF presents the annual cycle of Tavg better than varres-CESM with about a 1$^\circ C$ underestimation. Varres-CESM overestimated Tavg by 2$^\circ C$ over the summer season and produced a similar magnitude underestimation over winter season, indicating larger annual temperature range over most regions.

When assessing the frequency distribution of JJA Tmax, both varres-CESM and WRF 27km match closely to the UW dataset over all study area except in Central Valley (CV). The failure to correctly capture CV Tmax is likely caused by the lack of irrigation cooling effect over this region. Future work will address this issue by adding an irrigation parameterization to varres-CESM so as to figure out the role irrigation played in regulating Tmax, and the overestimation and longer upbounded tail of frequency distribution for Tmax,

As for representation of precipitation, varres-CESM matched closely with PRISM everywhere except for an overestimation of winter and annual precipitation (about 40$\%$-50$\%$) along the western side of Sierra Nevada and into the CV.  Increasing the spatial resolution to 0.125$^\circ$ produced a slight reduction in this overestimation (10$\%$) likely due to improved treatment of orographic effects. WRF 27km underestimates precipitation (by about 30$\%$) along the North Coast and Sierra Nevada mountains (where almost all the precipitation appears), whereas WRF 9km shows a large overestimation (about 70$\%$$-$80$\%$). The standard deviation of precipitation ranges from 0 to 6 mm$/$day, with generally higher interannual variability over locations of higher mean precipitation. When assessing the frequency of strong precipitation events, varres-CESM matches closely to the UW dataset everywhere except the CV.
...
Higher resolution (0.125$^\circ$) simulation of varres-CESM does produce better results in capturing summer Tmax and precipitation, along with their variablility, than the coarser resolution run. However, the improvements are not statistically significant. For WRF, when resolution is increased to 9km, the model produces vastly overestimated precipitation, as previous studies have also found when using RCMs for fine-scale regional simulations. The use of a convective parameterization is likely not needed in these simulations when grid spacing is near 10km, particularly since almost all of the precipitation comes from resolved (large-scale) condensation. In this sense, model deviation is more strongly related with resolved-scale processes and the choice of microphysics scheme plays a major role, motivating the need for more work on scale-aware parameterizations. 
...
\begin{table}
\caption{MSD (left column minus top row) of JJA temperature ($^\circ$C)and DJF precipitation (mm/d) between all reference datasets.  Statistically significant differences (at the 95\%) level are highlighted in red.} \label{tab:stat_Datasets}
\begin{tabular}{lcccc}
\hline \textbf{JJA Tmin} & \textbf{PRISM} & \textbf{UW} & \textbf{Daymet} \\
\hline & MR  $\ $ CV $\ $  DR $\ $  SC $\ $  NC & MR $\ $  CV  $\ $ DR  $\ $ SC $\ $  NC & MR  $\ $ CV  $\ $ DR  $\ $ SC $\ $  NC \\
\hline \textbf{UW} & {\color{red}-2.0 $\ $ -0.5} $\ $ -0.3 $\ $ {\color{red}-0.4 $\ $ -2.3} \\
\textbf{Daymet} & {\color{red}-1.8} $\ $ 0.2 $\ $ -0.2 $\ $ -0.3 $\ $ {\color{red}-4.6} & 0.2 $\ $ {\color{red}0.8} $\ $ 0.1 $\ $ 0.1 $\ $ {\color{red}-2.3} \\
\textbf{NARR} & {\color{red}2.5 $\ $ 2.5 $\ $ 3.1} $\ $ -0.2 $\ $ {\color{red}2.0} & {\color{red}4.5 $\ $ 3.1 $\ $ 3.4} $\ $ 0.2 $\ $ {\color{red}4.2} & {\color{red}4.3 $\ $ 2.3 $\ $ 3.2} $\ $ 0.1 $\ $ {\color{red}6.5} \\
\hline 
\end{tabular}

\begin{tabular}{lcccc}
\hline \textbf{JJA Tmax} & \textbf{PRISM} & \textbf{UW} & \textbf{Daymet} \\
\hline & MR  $\ $ CV $\ $  DR $\ $  SC $\ $  NC & MR $\ $  CV  $\ $ DR  $\ $ SC $\ $  NC & MR  $\ $ CV  $\ $ DR  $\ $ SC $\ $  NC \\
\hline \textbf{UW} & 0.3 $\ $ -0.1 $\ $ -0.2 $\ $ {\color{red}-0.7 $\ $ -2.4} \\
\textbf{Daymet} & -0.3 $\ $ 0.4 $\ $ -0.2 $\ $ 0.2 $\ $ {\color{red}-3.7} & -0.6 $\ $ {\color{red}0.5} $\ $ 0.1 $\ $ {\color{red}0.9 $\ $ -1.3} \\
\textbf{NARR} & {\color{red}-1.3 $\ $ 1.2} $\ $ -0.1 $\ $ {\color{red}-0.6 $\ $ -3.7} & {\color{red}-1.7 $\ $ 1.2} $\ $ 0.1 $\ $ 0.1 $\ $ {\color{red}-1.3} & {\color{red}-1.1 $\ $ 0.7} $\ $ 0.1 $\ $ {\color{red}-0.7} $\ $ 0.0 \\
\hline 
\end{tabular}
...
\textbf{NARR} & -0.5 $\ $ -0.4 $\ $ -0.1 $\ $ -0.1 $\ ${\color{red}-1.3} & -0.3 $\ $ -0.5 $\ $ -0.1 $\ $ 0.1 $\ $ -1.1 & -0.4 $\ $ -0.0 $\ $ -0.1 $\ $ -0.3 $\ $ {\color{red}-1.8} \\ %& 0.2 $\ $ -0.5 $\ $ -0.1 $\ $ 0.0 $\ $ -0.2 \\
\hline
\end{tabular}
\end{table}
...
\begin{table}
\begin{center}
\caption{RMSD, MSD and Spatial Correlation (Corr) for seasonally-averaged JJA temperature metrics over California. } \label{tab:stat_JJA_t2}
...
\hline \textbf{varres-CESM 0.25$^\circ$} & 2.322 $\ $ 3.745 & 2.924 $\ $ 3.121 $\ $ 2.604 & 2.810 $\ $ 3.934 \\
\textbf{varres-CESM 0.125$^\circ$} \quad & 1.900 $\ $ 3.631 & 2.447 $\ $ 2.944 $\ $ 2.184 & 2.475 $\ $ 3.701 \\
...
\textbf{Uniform CESM 1$^\circ$} & 3.063 $\ $ 4.591 & 3.621 $\ $ 3.428 $\ $ 3.161 & 3.583 $\ $ 5.071 \\
\hline
\end{tabular*}
...
\hline $    $ & T$_{max}$ $\     $  T$_{min}$ & T$_{max}$ $\     $  T$_{min}$ $\     $ T$_{avg}$& T$_{max}$ $\     $  T$_{min}$\\
\hline \textbf{varres-CESM 0.25$^\circ$} & 0.981 $\ $ 2.907 & 0.606 $\ $ 1.731 $\ $ 0.823 & 1.177 $\ $ 2.877 \\
\textbf{varres-CESM 0.125$^\circ$} \quad & 0.645 $\ $ 2.848 & 0.203 $\ $ 1.660 $\ $ 0.579 & 0.818 $\ $ 2.744 \\
...
\textbf{Uniform CESM 1$^\circ$} & 0.822 $\ $ 3.033 & 0.595 $\ $ 1.758 $\ $ 1.080 & 1.243 $\ $ 3.379 \\
\hline
\end{tabular*}
...
\hline $    $ & T$_{max}$ $\     $  T$_{min}$ & T$_{max}$ $\     $  T$_{min}$ $\     $ T$_{avg}$& T$_{max}$ $\     $  T$_{min}$\\
\hline \textbf{varres-CESM 0.25$^\circ$} & 0.998 $\ $ 0.982 & 0.996 $\ $ 0.986 $\ $ 0.994 & 0.997 $\ $ 0.979 \\
\textbf{varres-CESM 0.125$^\circ$} \quad & 0.998 $\ $ 0.985 & 0.997 $\ $ 0.988 $\ $ 0.996 & 0.997 $\ $ 0.983 \\
...
\textbf{Uniform CESM 1$^\circ$} & 0.995 $\ $ 0.964 & 0.993 $\ $ 0.979 $\ $ 0.991 & 0.994 $\ $ 0.955 \\
\hline
\end{tabular*}
\end{center}
\end{table}
...
\hline \textbf{} & \textbf{Central valley}  & \textbf{Mountain}\\
...
\hline\textbf{varres-CESM 0.25$^\circ$}& 34.1$\ $	26.2	$\ $-0.4$\ $	0.2	&	28.1	$\ $27.6$\ $	-0.4$\ $	0.3	\\
\textbf{varres-CESM 0.125$^\circ$}& 34.3	$\ $28.5$\ $	-0.5	$\ $0.4	&	27.2	$\ $30.0$\ $	-0.4$\ $	0.3	\\
...
\hline \textbf{} & \textbf{North coast} & \textbf{South coast} &\textbf{Desert} \\
...
\hline\textbf{varres-CESM 0.25$^\circ$}& 26.4	$\ $37.4	$\ $0.1$\ $	-0.7		& 26.4$\ $	37.4	$\ $0.1$\ $	-0.7	&	37.6$\ $	19.0$\ $	-0.5	$\ $0.8\\
\textbf{varres-CESM 0.125$^\circ$} & 26.3	$\ $37.4$\ $	0.1$\ $	-0.6		& 26.3$\ $	37.4$\ $	0.1	$\ $-0.6		& 37.3	$\ $21.3$\ $	-0.5	$\ $0.4\\
...
\begin{tabular}{p{6in}}
\small\textbf{Notes:} If skew $>0$ [skew $<0$], the distribution trails off to the right [left]. If kurtosis $> 0$ [$<0$], it is usually more sharply peaked [flatter] than the normal distribution (leptokurtic and platykurtic, respectively).
\end{tabular}
\end{table}
%\end{landscape}
%%%%%%Table4%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\hline \textbf{varres-CESM 0.25$^\circ$} & 0.607 $\ $ 0.394 $\ $ 0.302 $\ $ 0.981 & 0.616 $\ $ 0.292 $\ $ 0.287 $\ $ 0.968 \\
\textbf{varres-CESM 0.125$^\circ$} & 0.469 $\ $ 0.207 $\ $ 0.236 $\ $ 0.980 & 0.526 $\ $ 0.115 $\ $ 0.238 $\ $ 0.970 \\
\textbf{WRF 27km} & 0.419 $\ $ -0.205 $\ $ 0.210 $\ $ 0.977 & 0.580 $\ $ -0.308 $\ $ 0.242 $\ $ 0.971 \\
\textbf{WRF 9km} & 2.226 $\ $ 1.485 $\ $ 0.965 $\ $ 0.957 & 2.052 $\ $ 1.393 $\ $ 0.851 $\ $ 0.964 \\
%\textbf{uniform CESM 0.25d} & 0.555 $\ $ 0.134 $\ $ 0.238 $\ $ 0.969 & 0.600 $\ $ 0.031 $\ $ 0.253 $\ $ 0.961 & 0.700 $\ $ -0.057 $\ $ 0.265 $\ $ 0.953 & 0.600 $\ $ -0.069 $\ $ 0.245 $\ $ 0.962 \\
\textbf{uniform CESM 1$^\circ$} & 1.972 $\ $ -1.567 $\ $ 0.999 $\ $ 0.941 & 2.313 $\ $ -1.701 $\ $ 0.999 $\ $ 0.905 \\
\hline
\end{tabular*}
...
\hline $    $ & RMSD $\ $ MSD $\ $ MRD $\ $ Corr & RMSD $\ $ MSD $\ $ MRD $\ $ Corr \\
\hline \textbf{varres-CESM 0.25$^\circ$} & 0.727 $\ $ 0.203 $\ $ 0.309 $\ $ 0.952 & 0.567 $\ $ 0.191 $\ $ 0.248 $\ $ 0.972 \\
\textbf{varres-CESM 0.125$^\circ$} & 0.624 $\ $ 0.045 $\ $ 0.255 $\ $ 0.961 & 0.504 $\ $ 0.027 $\ $ 0.220 $\ $ 0.973 \\
\textbf{WRF 27km} & 0.765 $\ $ -0.396 $\ $ 0.273 $\ $ 0.965 & 0.647 $\ $ -0.409 $\ $ 0.274 $\ $ 0.970 \\
\textbf{WRF 9km} & 1.889 $\ $ 1.322 $\ $ 0.775 $\ $ 0.970 & 2.005 $\ $ 1.306 $\ $ 0.762 $\ $ 0.961 \\
%\textbf{uniform CESM 0.25d} & 0.555 $\ $ 0.134 $\ $ 0.238 $\ $ 0.969 & 0.600 $\ $ 0.031 $\ $ 0.253 $\ $ 0.961 & 0.700 $\ $ -0.057 $\ $ 0.265 $\ $ 0.953 & 0.600 $\ $ -0.069 $\ $ 0.245 $\ $ 0.962 \\
\textbf{uniform CESM 1$^\circ$} & 2.525 $\ $ -1.829 $\ $ 0.999 $\ $ 0.901 & 2.305 $\ $ -1.797 $\ $ 0.999 $\ $ 0.935 \\
\
\end{tabular*}
...
\hline $    $ & RMSD $\ $ MSD $\ $ MRD $\ $ Corr & RMSD $\ $ MSD $\ $ MRD $\ $ Corr \\
\hline \textbf{varres-CESM 0.25$^\circ$} & 1.486 $\ $ 0.986 $\ $ 0.361 $\ $ 0.972 & 1.445 $\ $ 0.673 $\ $ 0.325 $\ $ 0.959 \\
\textbf{varres-CESM 0.125$^\circ$} & 1.194 $\ $ 0.638 $\ $ 0.294 $\ $ 0.976 & 1.234 $\ $ 0.346 $\ $ 0.270 $\ $ 0.965 \\
\textbf{WRF 27km} & 0.888 $\ $ -0.376 $\ $ 0.212 $\ $ 0.975 & 1.289 $\ $ -0.688 $\ $ 0.258 $\ $ 0.967 \\
\textbf{WRF 9km} & 4.264 $\ $ 2.607 $\ $ 0.856 $\ $ 0.950 & 3.835 $\ $ 2.315 $\ $ 0.698 $\ $ 0.955 \\
%\textbf{uniform CESM 0.25d} & 1.392 $\ $ 0.377 $\ $ 0.286 $\ $ 0.960 & 1.431 $\ $ 0.064 $\ $ 0.283 $\ $ 0.951 & 1.544 $\ $ -0.033 $\ $ 0.293 $\ $ 0.946 & 1.406 $\ $ -0.095 $\ $ 0.275 $\ $ 0.953 \\
\textbf{uniform CESM 1$^\circ$} & 3.971 $\ $ -3.123 $\ $ 0.999 $\ $ 0.936 & 4.799 $\ $ -3.499 $\ $ 0.999 $\ $ 0.904 \\
\hline
\end{tabular*}
...
\hline  & \textbf{PRISM} & \textbf{Daymet} \\
\hline $    $ & RMSD $\ $ MSD $\ $ MRD $\ $ Corr & RMSD $\ $ MSD $\ $ MRD $\ $ Corr \\
\hline \textbf{varres-CESM 0.25$^\circ$} & 1.654 $\ $ 0.577 $\ $ 0.354 $\ $ 0.943 & 1.346 $\ $ 0.514 $\ $ 0.284 $\ $ 0.964 \\
\textbf{varres-CESM 0.125$^\circ$} & 1.395 $\ $ 0.287 $\ $ 0.292 $\ $ 0.955 & 1.170 $\ $ 0.212 $\ $ 0.251 $\ $ 0.969 \\
\textbf{WRF 27km} & 1.552 $\ $ -0.785 $\ $ 0.275 $\ $ 0.962 & 1.351 $\ $ -0.848 $\ $ 0.277 $\ $ 0.966 \\
\textbf{WRF 9km} & 3.570 $\ $ 2.256 $\ $ 0.663 $\ $ 0.964 & 3.804 $\ $ 2.183 $\ $ 0.653 $\ $ 0.955 \\
%\textbf{uniform CESM 0.25d} & 1.392 $\ $ 0.377 $\ $ 0.286 $\ $ 0.960 & 1.431 $\ $ 0.064 $\ $ 0.283 $\ $ 0.951 & 1.544 $\ $ -0.033 $\ $ 0.293 $\ $ 0.946 & 1.406 $\ $ -0.095 $\ $ 0.275 $\ $ 0.953 \\
\textbf{uniform CESM 1$^\circ$} & 5.074 $\ $ -3.648 $\ $ 0.999 $\ $ 0.902 & 4.685 $\ $ -3.651 $\ $ 0.999 $\ $ 0.935 \\
\hline
\end{tabular*}
...
\caption{Grid meshes for the two varres-CESM simulations.}
\label{fig:varres-CESM_map}
\end{center}
\end{figure}
...
\end{center}
\caption{Topography in meters (m) for (top left to bottom right) varres-CESM 0.25$^\circ$, varres-CESM 0.125$^\circ$, uniform CESM 1$^\circ$, WRF 27km, WRF 9km and ERA-Interim ($\sim$80 km).} \label{fig:topo} 
\end{figure}
...
%add description of Uniform CESM 1deg and its results
%divide methodology part into subsections?
%divide result part into subsections?
%add the supplement: appendix in the tex file?
