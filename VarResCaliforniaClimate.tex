...
The spatial standard deviation of JJA Tmax, Tmin and Tavg by models and PRISM are depicted in Figure \ref{fig:t2_JJA_std}. It can be seen that the variability is largely consistent across different sub-zones, and the values are around 0.5 to 1.5 $^\circ C$ for all the datasets, except for the high Sierras in the WRF 9km simulation which show enhanced variability  ($\sim$2$^\circ C$).
...
Overall, varres-CESM 0.125$^\circ$ performs slightly better than CESM 0.25$^\circ$ and WRF 27km, as further exhibited by the RMSD values in Table \ref{tab:stat_Pr}. 
...
\end{center}
\caption{Seasonal cycle of monthly-average Tavg for each subzone ($^\circ C$).} \label{fig:trd_t2avg_allzones}
\end{figure}
