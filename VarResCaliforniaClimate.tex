...
%{\color{red} To assess differences in these data products, we have used the Student's t-test to see if the JJA Tmax and Tmin from PRISM, UW and Daymet are statistically different from one another comparing seasonally averaged values. For these quantities, all products exhibited similarity at the 0.05 significance level over scattered regions of the study area.  A similar assessment was performed for mean DJF precipitation climatology and, in this case, all products exhibited similarity at the 0.05 significance level over the almost entire simulation domain. These results further proved that uncertainty exists within these different products of gridded observation datasets. [Unclear language.  What are scattered regions?  What is ``almost the entire simulation domain''?  Tabulated differences are much more useful.]}
...
%\begin{center}
%\begin{tabular*}{5in}{l @{\extracolsep{\fill}}cccc}
%\hline \textbf{Tmin} & \textbf{PRISM} & \textbf{UW} & \textbf{Daymet} & \textbf{NARR} \\
%\hline & MR  $\ $ CV $\ $  DR $\ $  SC $\ $  NC & MR $\ $  CV  $\ $ DR  $\ $ SC $\ $  NC & MR  $\ $ CV  $\ $ DR  $\ $ SC $\ $  NC & MR  $\ $ CV  $\ $ DR  $\ $ SC  $\ $ NC \\
%\hline \textbf{PRISM}  \\
%\textbf{UW} & {\color{red}-2.0 $\ $ -0.5} $\ $ -0.3 $\ $ {\color{red}-0.4 $\ $ -2.3} \\
...
%
%\begin{tabular*}{5in}{l @{\extracolsep{\fill}}cccc}
%\hline \textbf{Tmax} & \textbf{PRISM} & \textbf{UW} & \textbf{Daymet} & \textbf{NARR} \\
%\hline & MR  $\ $ CV $\ $  DR $\ $  SC $\ $  NC & MR $\ $  CV  $\ $ DR  $\ $ SC $\ $  NC & MR  $\ $ CV  $\ $ DR  $\ $ SC $\ $  NC & MR  $\ $ CV  $\ $ DR  $\ $ SC  $\ $ NC \\
%\hline \textbf{PRISM}  \\
%\textbf{UW} & 0.3 $\ $ -0.1 $\ $ -0.2 $\ $ {\color{red}-0.7 $\ $ -2.4} \\
...
%\end{tabular*}
%
%\begin{tabular*}{5in}{l @{\extracolsep{\fill}}c}
...
%\end{tabular*}
%
%\begin{tabular*}{5in}{l @{\extracolsep{\fill}}ccccc}
%\hline \textbf{Pr} & \textbf{PRISM} & \textbf{UW} & \textbf{Daymet} & \textbf{CPC} & \textbf{NARR} \\
%\hline & MR  $\ $ CV $\ $  DR $\ $  SC $\ $  NC & MR $\ $  CV  $\ $ DR  $\ $ SC $\ $  NC & MR  $\ $ CV  $\ $ DR  $\ $ SC $\ $  NC & MR  $\ $ CV  $\ $ DR  $\ $ SC  $\ $ NC & MR  $\ $ CV  $\ $ DR  $\ $ SC  $\ $ NC \\
%\hline \textbf{PRISM} \\
%\textbf{UW} & -0.2 $\ $ 0.1 $\ $ 0.0 $\ $ -0.2 $\ $ -0.2 \\
...
%\begin{table}
%\begin{center}
%\caption{RMSD and Correlation (Corr) for sample standard deviation of seasonally-averaged JJA temperature over California} \label{tab:stat_std_JJA_t2}
%\begin{tabular}{lcccc}
%\hline \textbf{RMSD} & \textbf{UW}  & \textbf{PRISM} & \textbf{Daymet} \\
%\hline $    $ & T$_{max}$ $\     $  T$_{min}$ & T$_{max}$ $\     $  T$_{min}$ $\     $ T$_{avg}$& T$_{max}$ $\     $  T$_{min}$\\
