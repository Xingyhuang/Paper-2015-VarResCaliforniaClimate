...
Standard statistical measures have been used to quantify the performances of the models in comparison with the reference datasets. These statistical variables include the Root-mean-square deviation (RMSD), mean signed difference (MAD), mean relative absolute difference (MRD) and spatial correlation, and sample standard deviation ($s$). Among those, correlation is the uncentered pattern correlation spatially, i.e. Pearson product-moment coefficient of linear correlation between models and reference datasets at corresponding locations.
\begin{equation} \label{eq:RMSD}
\mbox{RMSD} = \sqrt{\frac{1}{N} \sum_{i=1}^{N} (v_i - \hat{v}_i)^2}
\end{equation}

\begin{equation} \label{eq:MSD}
\mbox{MSD} = \frac{1}{N} \sum_{i=1}^{N} (v_i - \hat{v}_i)
\end{equation}

\begin{equation} \label{eq:MRD}
\mbox{MRD} = (\sum_{i=1}^{N} |v_i - \hat{v}_i|)/(\sum_{i=1}^{N} \hat{v}_i)
\end{equation}

\begin{equation} \label{eq:SSTD}
\mbox{$s$} = \sqrt{\frac{1}{M-1} \sum_{j=1}^{M} (v_j - \bar{v})^2}.
\end{equation}

where $v$ and $\hat{v}$ are values from models and reference datasets respectively; $i$ is the index of grid point within the study area CA, and N is the total number of grid points; $j$ is the index of year and M is the number of years. Grid point differences are calculated by remapping the reference datasets to the model's output grid using bilinear interpolation.  Remapping using patch-based interpolation has also been tested and does not exhibit notable differences. In some cases, values are further averaged over each regional zone.
...
In this study, student's t-test has been used to test the null hypothesis that two sets of yearly or seasonally or monthly averaged data are the same. F-test is applied to test the null hypothesis that the sample variances are equal. T-test and F-test are used only when the sample population can be described adequately by a normal distribution, and the normality is examined under the AndersonÐDarling test. When the sample population do not approximately follow a normal distribution, Mann-Whitney-Wilcoxon (MWW) test and and Levene's test are employed to substitute for t-test and F-test respectively. All these tests are evaluated at the 0.05 significance level.
...
We also provide complementary results in the supplement, including the original grid-refined mesh file we used, the variability over different time period, the t-test of the time trend, other seasons not shown in this papers and corresponding metric tables. We also compared varres-CESM with the output from a globally uniform CESM run at 0.25$^\circ$ spatial resolution with the finite volume (FV) dynamical core \cite{wehner2014effect}. However, since this uniform CESM run has different dynamical core from our runs, we also put this part in the supplement instead of explicitly displaying it in this paper.
...
For Tmax, when compared with the reference datasets, varres-CESM showed a warm bias of about 2 to 3$^\circ C$. However, WRF produced an overall colder climate, especially the WRF 9km simulation, which were about 2 to 3$^\circ C$ cooler than PRISM. Tmax over CV has been overestimated by all the simulations.  Possible reasons for this overestimation are discussed at the end of this section.
...
The spatial standard deviation of JJA Tmax, Tmin and Tavg by models and PRISM are depicted in Figure \ref{fig:t2_JJA_std}. It can be seen that the variability is largely consistent across different sub-zones, and the values are around 0.5 to 1.5 $^\circ C$ for all the datasets, except for the high Sierras in the WRF 9km simulation which show enhanced variability ($\sim$2$^\circ C$). From the RMSD values shown in Table \ref{tab:stat_std_JJA_t2}, varres-CESM and WRF 27km have smaller deviations ($\sim$0.1-0.2$^\circ C$) from observations comparing with WRF 9km ($\sim$0.2-0.3$^\circ C$).
...
The seasonal cycle of Tavg is shown in Figure \ref{fig:trd_t2avg_allzones} for simulations and reference data from PRISM and NARR. The values are calculated by taking the average of mean monthly Tavg over specific regional zone. Modeled results match closely with reference data with no larger than a 2$^\circ C$ difference, with the largest errors occurring in the summer and winter seasons showing statistical significance of differences over most regions and falling off the 0.95 confidence intervals of reference data. Compared with PRISM, Varres-CESM overpredicted summer season temperatures in all sub-zones except coastal regions, and underpredicted winter season temperatures in all zones, corresponding to a larger annual temperature range. WRF has better performance in presenting the monthly trend than CESM, which is further proved by the t-test, with about 1$^\circ C$ underestimation over all seasons. There is no clear improvement in the seasonal cycle across resolutions. There is as largest as $\sim$ 3$^\circ C$ difference between NARR and PRISM over south coast during JJA, which may due to the assimilation of strong coastal cooling effect within NARR or the lack of enough in-field observations.
...
Variability in monthly average Tavg is expressed by the sample standard deviation showed in Figure \ref{fig:trd_t2avg_allzones_std}. The variability is the interannual standard deviation of monthly Tavg over 26 years. Generally, standard deviation is between 1 to 2$^\circ C$. Among the simulations, WRF 27km is most consistent to PRISM with smallest difference . WRF 9km is also close to PRISM, but has $\sim$1$^\circ C$ larger variability over January and February with statistical significance of difference. Varres-CESM basically showed about 0.5$^\circ C$ more scattered values (either above or lower) comparing to reference datasets. The F-test results showed that most of the variability values among these models do not have statistically significant differences.
...
\begin{table}
\begin{center}
\caption{RMSD, MSD and Correlation (Corr) for seasonally-averaged JJA temperature metrics over California} \label{tab:stat_JJA_t2}
\begin{tabular}{lcccc}
...
\begin{table}
\begin{center}
\caption{RMSD, MSD, MRD, Correlation (Corr) for precipitation over California} \label{tab:stat_Pr}
\begin{tabular}{lcccc}
...
\end{center}
\caption{Topography in meters (m) for (top left to bottom right) varres-CESM 0.25$^\circ$, varres-CESM 0.125$^\circ$, WRF 27km, WRF 9km and ERA-Interim ($\sim$80 km).} \label{fig:topo} 
\end{figure}
