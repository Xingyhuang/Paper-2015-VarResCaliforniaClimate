...
Additionally, the following physics parameterizations were employed: WSM (WRF Single-Moment) 6-class graupel microphysics scheme \citep{hong2006wrf}, Kain-Fritsch cumulus scheme \citep{kain2004kain}, CAM shortwave and longwave radiation schemes \citep{collins2004description}. Part of these settings are chosen loosely from the results of one-year test runs over 1979 with combinations of different options including using Grell-Freitas scheme as cumulus scheme, and RRTM scheme or Dudhia scheme for the radiation schemes. We applied the options that gave the best results after comparing with gridded observations in our study. For the boundary layer, the Yonsei University scheme (YSU) \citep{hong2006new} and the Noah Land Surface Model \citep{chen2001coupling} were used. Both were chosen as they are common for climate applications that balance long-term reliability and computational cost.  Although many other options and combinations of parameterizations are available for configuring WRF (and others have tackled a complete assessment of these options for particular problems), this choice was made simply to represent a typical RCM configuration.
...
To assess differences in these data products, we have calculated the MSD values between the above reference datasets for seasonally averaged JJA Tmax, Tmin and DJF Pr over five sub-zones. We have also used the Student's t-test to see if the those differences are statistically significant. The MSD values are showed in Table \ref{tab:stat_Datasets}, and the ones with statistically significant difference are marked in red. For mean temperature, gridded observational datasets are different from each other over some regions, especially for Tmin, with MSD values reaching up to $\sim$ 4$^\circ C$. UW and Daymet have colder climatology than PRISM. NARR as a reanalysis dataset is different from others over most regions with overall larger Tmin and smaller Tmax. For precipitation, basically no significant difference showed between these datasets, especially among PRISM, UW and Daymet. NARR and CPC seem to have lower precipitation values than others. Those results further proved that uncertainty exists within these different products.
...
\begin{landscape}
\begin{table}
\caption{MSD of JJA temperature and DJF precipitation between all reference datasets} \label{tab:stat_Datasets}
\begin{tabular}{l cccc}
\hline \textbf{Tmin} & \textbf{PRISM} & \textbf{UW} & \textbf{Daymet} \\
...
\begin{tabular}{lcccc}
\hline \textbf{Tmax} & \textbf{PRISM} & \textbf{UW} & \textbf{Daymet} \\
\hline & MR  $\ $ CV $\ $  DR $\ $  SC $\ $  NC & MR $\ $  CV  $\ $ DR  $\ $ SC $\ $  NC & MR  $\ $ CV  $\ $ DR  $\ $ SC $\ $  NC \\
...
\hline \textbf{Pr} & \textbf{PRISM} & \textbf{UW} & \textbf{Daymet} & \textbf{CPC} \\
...
%\end{center}
\begin{tabular}{p{6in}}
\textbf{Notes:} The negative values means that dataset specified in the left column is smaller than the one stated in the top row. Values with statistically significant difference are marked in the red color.
\end{tabular}
\end{table}
\end{landscape}
...
\textbf{uniform CESM 1deg} & 3.063 $\ $ 4.591 & 3.621 $\ $ 3.428 $\ $ 3.161 & 3.583 $\ $ 5.071 \\
\hline
\end{tabular*}
...
\textbf{uniform CESM 1deg} & 0.822 $\ $ 3.033 & 0.595 $\ $ 1.758 $\ $ 1.080 & 1.243 $\ $ 3.379 \\
\hline
\end{tabular*}
...
\textbf{uniform CESM 1deg} & 0.995 $\ $ 0.964 & 0.993 $\ $ 0.979 $\ $ 0.991 & 0.994 $\ $ 0.955 \\
\hline
\end{tabular*}
\end{center}
\end{table}
...
\begin{tabular*}{9in}{l @{\extracolsep{\fill}}ccccc}
...
\textbf{uniform CESM 1deg} & 1.972 $\ $ -1.567 $\ $ 0.999 $\ $ 0.941 & 2.313 $\ $ -1.701 $\ $ 0.999 $\ $ 0.905 & 2.525 $\ $ -1.829 $\ $ 0.999 $\ $ 0.901 & 2.305 $\ $ -1.797 $\ $ 0.999 $\ $ 0.935 \\
\hline
\end{tabular*}
...
\textbf{uniform CESM 1deg} & 3.971 $\ $ -3.123 $\ $ 0.999 $\ $ 0.936 & 4.799 $\ $ -3.499 $\ $ 0.999 $\ $ 0.904 & 5.074 $\ $ -3.648 $\ $ 0.999 $\ $ 0.902 & 4.685 $\ $ -3.651 $\ $ 0.999 $\ $ 0.935 \\
\hline
\end{tabular*}
\end{center}
\end{table}
